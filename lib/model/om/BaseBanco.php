...
abstract class BaseBanco extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the sucursal field.
	 * @var        string
	 */
	protected $sucursal;
	...
	/**
	 * Set the value of [sucursal] column.
	 * 
	 * @param      string $v new value
	 * @return     Banco The current object (for fluent API support)
	 */
	public function setSucursal($v)
	{
		...
		if ($this->sucursal !== $v) {
			$this->sucursal = $v;
			$this->modifiedColumns[] = BancoPeer::SUCURSAL;
		}
	...
	public function setDireccion($v)
	{
		...
		if ($this->direccion !== $v) {
			$this->direccion = $v;
			$this->modifiedColumns[] = BancoPeer::DIRECCION;
		}
	...
	public function setWeb($v)
	{
		...
		if ($this->web !== $v) {
			$this->web = $v;
			$this->modifiedColumns[] = BancoPeer::WEB;
		}
	...
	public function setFechaalta($v)
	{
		...
		if ( $this->fechaalta !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = BancoPeer::FECHAALTA;
			}
	...
	public function setFechabaja($v)
	{
		...
		if ( $this->fechabaja !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = BancoPeer::FECHABAJA;
			}
		} // if either are not null
	...
	public function setHabilitado($v)
	{
		...
		if ($this->habilitado !== $v || $this->isNew()) {
			$this->habilitado = $v;
			$this->modifiedColumns[] = BancoPeer::HABILITADO;
		}
	...
	public function setUsuario($v)
	{
		...
		if ($this->usuario !== $v) {
			$this->usuario = $v;
			$this->modifiedColumns[] = BancoPeer::USUARIO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->sucursal = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 9; // 9 = BancoPeer::NUM_COLUMNS - BancoPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setSucursal($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BancoPeer::SUCURSAL)) $criteria->add(BancoPeer::SUCURSAL, $this->sucursal);
		if ($this->isColumnModified(BancoPeer::DIRECCION)) $criteria->add(BancoPeer::DIRECCION, $this->direccion);
		if ($this->isColumnModified(BancoPeer::WEB)) $criteria->add(BancoPeer::WEB, $this->web);
		if ($this->isColumnModified(BancoPeer::FECHAALTA)) $criteria->add(BancoPeer::FECHAALTA, $this->fechaalta);
		if ($this->isColumnModified(BancoPeer::FECHABAJA)) $criteria->add(BancoPeer::FECHABAJA, $this->fechabaja);
		if ($this->isColumnModified(BancoPeer::HABILITADO)) $criteria->add(BancoPeer::HABILITADO, $this->habilitado);
		if ($this->isColumnModified(BancoPeer::USUARIO)) $criteria->add(BancoPeer::USUARIO, $this->usuario);
	...
	public function getUsuarioRelatedByUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuarioRelatedByUsuario === null && ($this->usuario !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aUsuarioRelatedByUsuario->addBancos($this);
			 */
		}
	 ...
	 * related Empresas from storage. If this Banco is new, it will return
		...
		if ($this->collEmpresas === null) {
			if ($this->isNew()) {
			   $this->collEmpresas = array();
				...
				$criteria->add(EmpresaPeer::IDBANCO, $this->id);
	 ...
	 */
	public function getEmpresasJoinUsuarioRelatedByUsuario($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(BancoPeer::DATABASE_NAME);
		}
		...
		if ($this->collEmpresas === null) {
			if ($this->isNew()) {
				...
				$this->collEmpresas = EmpresaPeer::doSelectJoinUsuarioRelatedByUsuario($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseBanco:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseBanco::%s', $method));
	  }
